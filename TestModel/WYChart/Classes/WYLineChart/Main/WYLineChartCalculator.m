...
//    CGFloat y;
    CGFloat maxValue = [[_parentView.yPoint lastObject] floatValue];//[[self filterPointsOfLinesPointSet:_parentView.points
//                                         comparatorBlock:^BOOL(WYLineChartPoint *firstPoint, WYLineChartPoint *secondPoint) {
//                                             return firstPoint.value > secondPoint.value;
//                                         }] value];
    CGFloat minValue = [[_parentView.yPoint firstObject] floatValue]; /*[[self filterPointsOfLinesPointSet:_parentView.points
                                          comparatorBlock:^BOOL(WYLineChartPoint *firstPoint, WYLineChartPoint *secondPoint) {
                                              return firstPoint.value < secondPoint.value;
                                          }] value];*/
...
//    CGFloat pixels = self.drawableAreaHeight - (_parentView.lineTopMargin + 30.f) - _parentView.lineBottomMargin;
    CGFloat perY =self.drawableAreaHeight/(_parentView.yPoint.count+1);
    if (value>maxValue) {
        return perY *0.5;
    }else if (value<minValue)
    {
        if (value==0) {
            return self.drawableAreaHeight;
        }
        return self.drawableAreaHeight -value*perY /minValue ;
    }else
    {
        return self.drawableAreaHeight -( (value -minValue)/differ *((_parentView.yPoint.count-1)*perY)+perY);
    }
